...
def test_vec_env_exposes_one_subenv_per_robot():
    vec_env = make_shared_box_push_vec_env(difficulty="easy", max_steps=50, num_robots=3)
    try:
        assert vec_env.num_envs == 3
        assert vec_env.observation_space.shape == (EGO_OBS_DIM,)
        assert vec_env.action_space.shape == (2,)
        observations = vec_env.reset()
        assert observations.shape == (3, EGO_OBS_DIM)
    finally:
        vec_env.close()
...
def test_step_returns_per_agent_arrays_and_shared_reward():
    ...
    try:
        ...
        assert len(infos) == 3
        assert [info["agent_index"] for info in infos] == [0, 1, 2]
    finally:
        vec_env.close()


def test_egocentric_observation_is_local_not_global():
    env = BoxPushEnv(difficulty="easy", max_steps=50, num_robots=3)
    try:
        env.reset()
        obs0 = egocentric_observation(env, 0)
        obs1 = egocentric_observation(env, 1)
        assert obs0.shape == (EGO_OBS_DIM,)
        # Robots at different places must see different local worlds,
...
def test_egocentric_observation_includes_fixed_neighbor_count():
    env = BoxPushEnv(difficulty="easy", max_steps=50, num_robots=3)
    try:
        env.reset()
        # Two neighbors -> 11 base + 2*3 neighbor + 3 geometry = 20 features.
        assert egocentric_observation(env, 0, num_neighbors=2).shape == (20,)
        assert egocentric_observation(env, 0, num_neighbors=1).shape == (17,)
    finally:
        env.close()


def test_short_rollout_runs_without_error():
    vec_env = SharedBoxPushVecEnv(BoxPushEnv(difficulty="easy", max_steps=10, num_robots=3))
    try:
        vec_env.reset()
