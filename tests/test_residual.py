...
def test_zero_residual_reproduces_expert():
    ...
    try:
        ...
        expected = np.asarray(expert.predict(observation), dtype=np.float32).reshape(4)
...
def test_residual_correction_is_scaled_and_added():
    ...
    try:
        ...
        expert = BoxPushHeuristicPolicy()
        expert_action = np.asarray(expert.predict(observation), dtype=np.float32).reshape(4)
